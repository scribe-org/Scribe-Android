...
internal class ScribeKey: Button {
    ...
    internal fun toEscape() {
        ...
        var selectKeyboardIconConfig = UIImage.SymbolConfiguration(
            pointSize = annotationHeight * 0.75, weight = .light, scale = .medium
        )
        if (DeviceType.isPad) {
            selectKeyboardIconConfig = UIImage.SymbolConfiguration(
                pointSize = annotationHeight * 1.1, weight = .light, scale = .medium
            )
        }
    ...
    internal fun setFullCornerRadius() {
        this.layer.borderColor = UIColor.clear.cgColor // border is set by the shadow
        this.layer.maskedCorners = listOf(
    ...
    internal fun setEscShadow() {
        this.shadow.backgroundColor = specialKeyColor
        this.shadow.layer.cornerRadius = commandKeyCornerRadius
        this.shadow.layer.maskedCorners = listOf(
            .layerMinXMinYCorner, .layerMinXMaxYCorner, .layerMaxXMinYCorner, .layerMaxXMaxYCorner
        )
        this.shadow.clipsToBounds = true
        this.shadow.layer.masksToBounds = false
