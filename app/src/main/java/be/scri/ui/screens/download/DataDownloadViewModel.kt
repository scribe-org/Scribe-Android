    ...
    fun handleDownloadAction(
        ...
        if (forceDownload) {
            ...
            if (currentState == DownloadState.Completed) {
                val template = getApplication<Application>().getString(R.string.i18n_app_download_menu_ui_download_data_already_up_to_date)
                ...
                try {
                    ...
                    if (forceDownload || isUpdateAvailable(localLastUpdate, serverLastUpdate)) {
                        ...
                        withContext(Dispatchers.Main) {
                            downloadStates[key] = DownloadState.Completed
                            val template = getApplication<Application>().getString(R.string.i18n_app_download_menu_ui_download_data_download_success)
                            val msg = StringUtils.formatStringWithParams(template, displayLang)
                            Toast.makeText(getApplication(), msg, Toast.LENGTH_SHORT).show()
                        }
                    } else {
